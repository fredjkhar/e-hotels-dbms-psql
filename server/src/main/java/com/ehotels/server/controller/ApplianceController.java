...
public class ApplianceController {
    ...
    public ResponseEntity<List<Appliance>> customApplianceControllerSelectQuery(@RequestBody String query) {
        List<Appliance> appliances = null;
        try {
            Objects.requireNonNull(query);
            appliances = service.customSelectQuery(query);
            return appliances.isEmpty() ? ResponseEntity.notFound().build() : ResponseEntity.ok(appliances);

        } catch (Exception e) {
            logger.error("Failed to execute select query: {}", query, e);
            // Return a 500 Internal Server Error if server failed to create an appliance record
            return ResponseEntity.internalServerError().body(appliances);
        }
    }
