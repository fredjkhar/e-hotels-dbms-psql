...
public class EmployeeService implements EmployeeRepo {
    ...
    public int insert(Employee employee) {
        String query = "INSERT INTO employee (employee_NAS, last_name, first_name, street_number, street, city, province, country, postal_code, salary, phone_number, email, password, role, overtime, hotel_id)"+
                "VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        return jdbcTemplate.update(query, employee.getEmployee_NAS(), employee.getLast_name(), employee.getFirst_name(), employee.getStreet_number(), employee.getStreet(), employee.getCity(), employee.getProvince(), employee.getCountry(), employee.getPostal_code(), employee.getSalary(), employee.getPhone_number(), employee.getEmail(), employee.getPassword(), employee.getRole(), employee.getOvertime(), employee.getHotel_id());
    }

    @Override
    public int update(Employee employee) {
        String query = "UPDATE employee SET last_name=?, first_name=?, street_number=?, street=?, city=?, province=?, country=?, postal_code=?, salary=?, phone_number=?, email=?, password=?, role=?, overtime=?, hotel_id=? WHERE employee_NAS=?";
